  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">{t('displaySettings')}</h1>
          <p className="text-muted-foreground">
            {t('language') === 'da' ? 'Tilpas grid layout til din skærmstørrelse' : 'Customize grid layout for your screen size'}
          </p>
        </div>
        <div className="flex gap-2">
          ...
          >
            {previewMode ? `⚙️ ${t('settings')}` : `👁️ ${t('preview')}`}
          </Button>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['small', 'medium', 'large'].map(size => (
                    <Button
                      ...
                      size="sm"
                      onClick={() => setCurrentSettings(prev => ({ ...prev, buttonSize: size as 'small' | 'medium' | 'large' }))}
                    >
                      {size === 'small' && '🔸 Lille'}
                      {size === 'medium' && '🔶 Mellem'}
                      {size === 'large' && '🔷 Stor'}
