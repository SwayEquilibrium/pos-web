...
-- ================================================
-- Check if categories table exists and add missing columns
do $$
begin
...
-- ================================================
-- MODIFIERS (align with existing system)
-- ================================================
do $$
begin
...
begin
  if not exists (select 1 from information_schema.tables where table_name = 'modifiers' and table_schema = 'public') then
    ...
    -- Add missing columns to existing modifiers
    ...
    -- Rename price_adjustment to price_delta if it exists
...
-- ================================================
-- PRODUCT MODIFIER GROUPS RELATIONSHIP
-- ================================================
do $$
begin
  if not exists (select 1 from information_schema.tables where table_name = 'product_modifier_groups' and table_schema = 'public') then
    ...
    -- Rename modifier_group_id to group_id if it exists
    if exists (select 1 from information_schema.columns where table_name = 'product_modifier_groups' and column_name = 'modifier_group_id') then
